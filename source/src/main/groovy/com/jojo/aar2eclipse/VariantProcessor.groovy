...
class VariantProcessor {
    ...
    private void processAssets() {
        ...
        // 把重复的asset删除
        assetsTask.doFirst {
            HashSet<String> addedAsset = new HashSet()
            for (archiveLibrary in mAndroidArchiveLibraries) {
                if (archiveLibrary.assetsFolder != null && archiveLibrary.assetsFolder.exists()) {
                    mProject.android.sourceSets.each {
                        if (it.name == mVariant.name) {
                            for (File assetFile in archiveLibrary.assetsFolder.listFiles()) {
                                String fName = assetFile.getName()
                                if (addedAsset.contains(fName)) {
                                    assetFile.delete()
                                } else {
                                    addedAsset.add(fName)
                                }
                            }
